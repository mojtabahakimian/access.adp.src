...
Begin Form
    ...
    RecSrcDt = Begin
        0xf84abd153336e440
    End
    RecordSource ="SELECT PAY_GETP.*, TCOD_BANKS.NAMES FROM PAY_GETP INNER JOIN TCOD_BANKS ON PAY_G"
        "ETP.BANK = TCOD_BANKS.CODE WHERE (PAY_GETP.N_KOL <> 911) AND (NOT (PAY_GETP.N_KO"
        "L2 IS NULL)) OR (PAY_GETP.N_KOL <> 911) AND (NOT (PAY_GETP.N_KOL3 IS NULL)) ORDE"
        "R BY PAY_GETP.N_SERI"
    Caption ="چكهاي پرداختي وصول شده يا برگشت شده"
