...
Begin Form
    ...
    RecordSource ="SELECT SUM(MEGHKF) AS MEGHF, SUM(MABK) AS MABKK, SUM(MEGHkK) AS MEGHk, SAIZE, TA"
        "RH, CAST(TARH AS INT) AS TTARH, DEGR, CODE, N_FANI, NAME FROM FROOSH_COUNTALL_CI"
        "TY_N_FANI(1, 999999) FROOSH_COUNTALL_CITY_N_FANI GROUP BY SAIZE, TARH, CAST(TARH"
        " AS INT), DEGR, CODE, N_FANI, NAME"
