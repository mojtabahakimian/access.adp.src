...
Begin Form
    ...
    RecordSource ="SELECT HEAD_LST.NUMBER, HEAD_LST.NUMBER1, HEAD_LST.CUST_NO, HEAD_LST.SADER, SUM("
        ...
        " MABL_KS, SUM(HEAD_LST.MBAA) AS MBAAs, SUM(HEAD_LST.TAKHFIF) AS TAKHFIFs FROM HE"
        "AD_LST INNER JOIN INVO_LST ON HEAD_LST.NUMBER = INVO_LST.NUMBER AND HEAD_LST.TAG"
        " - 11 = INVO_LST.TAG INNER JOIN HEAD_LST HEAD_LST_1 ON INVO_LST.NUMBER = HEAD_LS"
        "T_1.NUMBER AND INVO_LST.TAG = HEAD_LST_1.TAG INNER JOIN CUST_HESAB ON HEAD_LST.C"
        "UST_NO = CUST_HESAB.hes WHERE (HEAD_LST.TAG = 13) GROUP BY HEAD_LST.NUMBER, HEAD"
        "_LST.NUMBER1, HEAD_LST.CUST_NO, HEAD_LST.SADER, HEAD_LST.DATE_N, HEAD_LST_1.DATE"
        "_N, CUST_HESAB.NAME, CUST_HESAB.hes, HEAD_LST.ANBARF, HEAD_LST_1.ANBAR"
