...
Begin Form
    ...
    RecordSource ="SELECT SUM(INVO_LST.MABL_K - INVO_LST.N_MOIN + INVO_LST.IMBAA) AS GMAB, HEAD_LST"
        ...
        "EAD_LST.SHIFT, HEAD_LST.CUST_KIND, HEAD_LST.USER_NAME, HEAD_LST.TAMIR FROM HEAD_"
        "LST INNER JOIN INVO_LST ON HEAD_LST.NUMBER = INVO_LST.NUMBER AND HEAD_LST.TAG = "
        "INVO_LST.TAG WHERE (HEAD_LST.TAG = 2) AND (HEAD_LST.TAMIR = 0)  and   (HEAD_LST."
        "CUST_NO = N'112-1-92') GROUP BY HEAD_LST.NUMBER, HEAD_LST.DATE_N, HEAD_LST.CUST_"
        "NO, HEAD_LST.MOLAH, HEAD_LST.DEPATMAN, HEAD_LST.SHIFT, HEAD_LST.CUST_KIND, HEAD_"
        "LST.USER_NAME, HEAD_LST.TAMIR"
