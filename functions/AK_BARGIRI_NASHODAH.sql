CREATE FUNCTION [dbo].[AK_BARGIRI_NASHODAH]
(@Forms___F_MENU_ANBAR___DT2 bigint,
@Forms___F_MENU_ANBAR___MANBAR nvarchar(10))
RETURNS TABLE
AS
RETURN ( SELECT     dbo.INVO_LST.CODE, SUM(dbo.INVO_LST.MEGHk) AS MEGBARG, AVG(dbo.INVO_LST.MABL) AS AvgOfMABL, dbo.INVO_LST.ANBAR
...
WHERE     (dbo.INVO_LST.TAG = 2) AND (dbo.HEAD_LST.DATE_N <= @Forms___F_MENU_ANBAR___DT2) AND (dbo.HEAD_LST.TAMIR = 0)
