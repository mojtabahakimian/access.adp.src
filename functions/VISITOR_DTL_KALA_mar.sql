...
  SELECT     TOP 100 PERCENT dbo.INVO_LST.CODE, SUM(dbo.INVO_LST.MEGHk / ISNULL((SELECT COUNT(*) FROM dbo.VISITOR_DTL WHERE NUMBER = dbo.INVO_LST.NUMBER AND TAG = dbo.INVO_LST.TAG), 1)) AS MEGHk, SUM(dbo.INVO_LST.MABL_K / ISNULL((SELECT COUNT(*) FROM dbo.VISITOR_DTL WHERE NUMBER = dbo.INVO_LST.NUMBER AND TAG = dbo.INVO_LST.TAG), 1)) AS MABL_K,
...
                        dbo.VISITOR_DTL.CUST_NO, SUM(dbo.INVO_LST.MEGHk / ISNULL((SELECT COUNT(*) FROM dbo.VISITOR_DTL WHERE NUMBER = dbo.INVO_LST.NUMBER AND TAG = dbo.INVO_LST.TAG), 1)) AS MEGH_MAR, SUM(dbo.INVO_LST.MABL_K / ISNULL((SELECT COUNT(*) FROM dbo.VISITOR_DTL WHERE NUMBER = dbo.INVO_LST.NUMBER AND TAG = dbo.INVO_LST.TAG), 1)) AS MABMAR,
