...
'14000509
  ...
  " ROUND(ISNULL(ISNULL(AKMOGO_AVL_KOL.SMEGH, 0) - ISNULL(AK_MOGO_FR.MEG, 0), 0) / ISNULL(dbo.FNESBAT.FNESBAT, 1), 0) AS MANDF, ISNULL(AK_BARGIRI_NASHODAH.MEGBARG, 0) AS MEGHBAR, ISNULL(AK_RESERVED.MEGHRES, 0) AS MEGHRES, dbo.STUF_DEF.B_SEF, dbo.STUF_DEF.MABL_F , ROUND(ISNULL(ISNULL(AKMOGO_AVL_KOL.SMEGH, 0) - ISNULL(AK_MOGO_FR.meg, 0), 0), 2) - dbo.STUF_DEF.B_SEF AS bsef, ROUND(ISNULL(ISNULL(AKMOGO_AVL_KOL.SMEGH, 0) - ISNULL(AK_MOGO_FR.MEG, 0), 0), 2) - dbo.STUF_DEF.N_SEF AS nsef, ROUND(ISNULL(ISNULL(AKMOGO_AVL_KOL.SMEGH, 0) - ISNULL(AK_MOGO_FR.MEG, 0), 0), 2) - dbo.STUF_DEF.MIN_M AS minm, ROUND(ISNULL(ISNULL(AKMOGO_AVL_KOL.SMEGH, 0) - ISNULL(AK_MOGO_FR.MEG, 0), 0), 2) - dbo.STUF_DEF.MAX_M AS maxm, dbo.STUF_DEF.MAX_M, dbo.STUF_DEF.VAZN, ROUND(ISNULL(ISNULL(AKMOGO_AVL_KOL.SMEGH, 0)  - ISNULL(AK_MOGO_FR.MEG, 0), 0), 2) * dbo.STUF_DEF.VAZN AS VAZNK, dbo.TCODE_MENUITEM.NAMES AS menuit," & _
...
'14001004
 ...
 " GROUP BY dbo.HEAD_LST.NUMBER, dbo.HEAD_LST.CUST_NO, dbo.HEAD_LST.DATE_N, dbo.HEAD_LST.TAG  HAVING      (dbo.HEAD_LST.DATE_N BETWEEN @DT1 AND @DT2)AND (dbo.HEAD_LST.TAG = 2) OR  (dbo.HEAD_LST.TAG = 13) OR (dbo.HEAD_LST.TAG = 14)  ORDER BY dbo.HEAD_LST.DATE_N DESC  UNION  SELECT     TOP (100) PERCENT dbo.HEAD_LST.NUMBER, dbo.HEAD_LST.CUST_NO, dbo.HEAD_LST.DATE_N, SUM(0) AS GHABEL,  SUM(dbo.INVO_LST.MABL_K - dbo.INVO_LST.N_MOIN + dbo.INVO_LST.IMBAA) AS MABMAR, SUM(0) AS MABL_K, SUM(0)  AS MEGHk, SUM(dbo.INVO_LST.MEGHk) AS MEGH_MAR, SUM(dbo.INVO_LST.N_MOIN) AS N_MOIN, SUM(dbo.INVO_LST.IMBAA) AS IMBAA,  dbo.HEAD_LST.TAG FROM         dbo.HEAD_LST INNER JOIN  dbo.INVO_LST ON dbo.HEAD_LST.NUMBER = dbo.INVO_LST.NUMBER AND dbo.HEAD_LST.TAG = dbo.INVO_LST.TAG GROUP BY dbo.HEAD_LST.NUMBER, dbo.HEAD_LST.CUST_NO, dbo.HEAD_LST.DATE_N, dbo.HEAD_LST.TAG " & _
  ...
  PLUSU
  CurrentProject.Connection.Execute ("CREATE FUNCTION dbo.VISITORS_KOL (@DT1 bigint, @DT2 bigint, @visitor nvarchar(40)) RETURNS Table AS RETURN ( WITH DistinctVisitorInvoice AS ( SELECT DISTINCT V.CUST_NO, V.NUMBER, V.TAG, V.mabpur FROM dbo.VISITOR_DTLO AS V WHERE V.CUST_NO LIKE @visitor ), " & _
 " VisitorInvoice AS ( SELECT V.CUST_NO, V.NUMBER, V.TAG, SUM(V.mabpur) AS mabpur FROM DistinctVisitorInvoice AS V GROUP BY V.CUST_NO, V.NUMBER, V.TAG ) " & _
 " SELECT VI.CUST_NO, SUM(VI.mabpur) AS mabpur, SUM(JAMFACTVISIT.MABL_K) AS MABL_K, SUM(JAMFACTVISIT.MEGHk) AS MEGHk, SUM(JAMFACTVISIT.MEGH_MAR) AS MEGH_MAR, SUM(JAMFACTVISIT.N_MOIN) AS TAKHF, SUM(JAMFACTVISIT.IMBAA) AS MBAA, " & _
 " SUM(JAMFACTVISIT.MABMAR) AS MABMAR, SUM(JAMFACTVISIT.GHABEL) AS GHABEL, dbo.CUST_HESAB.NAME, dbo.CUST_HESAB.ADDRESS, dbo.CUST_HESAB.TEL, dbo.CUST_HESAB.TOZIH, dbo.CUST_HESAB.MOBILE, COUNT(*) AS tedad FROM dbo.CUST_HESAB " & _
 " INNER JOIN VisitorInvoice AS VI ON dbo.CUST_HESAB.hes = VI.CUST_NO INNER JOIN dbo.JAMFACTVISIT(@DT1, @DT2) JAMFACTVISIT ON VI.NUMBER = JAMFACTVISIT.NUMBER AND VI.TAG = JAMFACTVISIT.TAG " & _
 " GROUP BY VI.CUST_NO, dbo.CUST_HESAB.NAME, dbo.CUST_HESAB.ADDRESS, dbo.CUST_HESAB.TEL, dbo.CUST_HESAB.TOZIH, dbo.CUST_HESAB.MOBILE ) ")

  CurrentProject.Connection.Execute ("CREATE VIEW VISITOR_DTLO AS SELECT dbo.VISITOR_DTL.NUMBER, dbo.VISITOR_DTL.TAG, dbo.VISITOR_DTL.CUST_NO, dbo.VISITOR_DTL.PURSANT - ISNULL(dbo.VISITOR_DTLR.PURSANT, 0) AS mabpur, dbo.VISITOR_DTL.PURSANT, dbo.VISITOR_DTL.TOZIH, dbo.VISITOR_DTL.STAT FROM dbo.VISITOR_DTL LEFT OUTER JOIN dbo.VISITOR_DTLR ON dbo.VISITOR_DTL.NUMBER = dbo.VISITOR_DTLR.NUMBER AND dbo.VISITOR_DTL.TAG = dbo.VISITOR_DTLR.tag AND dbo.VISITOR_DTL.CUST_NO = dbo.VISITOR_DTLR.CUST_NO WHERE (dbo.VISITOR_DTL.TAG = 2)")
  PLUSU
     CurrentProject.Connection.Execute ("ALTER TABLE [dbo].[GPSLOCATION] ALTER COLUMN  [LATITUDE]  [real]")
    PLUSU
    CurrentProject.Connection.Execute ("ALTER TABLE [dbo].[GPSLOCATION] ALTER COLUMN  [LONGITUDE]  [real]")
