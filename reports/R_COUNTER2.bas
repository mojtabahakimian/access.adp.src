...
Begin Report
    ...
    RecordSource ="SELECT STUF_DEF.CODE, STUF_DEF.NAME, STUF_FSK.ANBAR, TCOD_ANBAR.NAMES, 1 AS Expr"
        "1, ANBARGRD_SUB2.GRD_NUM, STUF_FSK.POSITION, TCOD_VAHEDS.NAMES AS vahed FROM STU"
        "F_DEF INNER JOIN STUF_FSK ON STUF_DEF.CODE = STUF_FSK.CODE INNER JOIN TCOD_ANBAR"
        " ON STUF_FSK.ANBAR = TCOD_ANBAR.CODE INNER JOIN ANBARGRD_SUB2 ON STUF_DEF.CODE ="
        " ANBARGRD_SUB2.CODE INNER JOIN TCOD_VAHEDS ON STUF_DEF.VAHED = TCOD_VAHEDS.CODE"
    Caption ="چاپ شمارش دوم"
