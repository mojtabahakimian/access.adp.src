...
Begin Report
    ...
    RecordSource ="SELECT HEAD_LST.NUMBER, HEAD_LST.NUMBER1, HEAD_LST.CUST_NO, HEAD_LST.SADER, SUM("
        ...
        " MABL_KS FROM HEAD_LST INNER JOIN INVO_LST ON HEAD_LST.NUMBER = INVO_LST.NUMBER "
        "AND HEAD_LST.TAG - 11 = INVO_LST.TAG INNER JOIN HEAD_LST HEAD_LST_1 ON INVO_LST."
        "NUMBER = HEAD_LST_1.NUMBER AND INVO_LST.TAG = HEAD_LST_1.TAG INNER JOIN CUST_HES"
        "AB ON HEAD_LST.CUST_NO = CUST_HESAB.hes WHERE (HEAD_LST.TAG = 13) GROUP BY HEAD_"
        "LST.NUMBER, HEAD_LST.NUMBER1, HEAD_LST.CUST_NO, HEAD_LST.SADER, HEAD_LST.DATE_N,"
        " HEAD_LST_1.DATE_N, CUST_HESAB.NAME, CUST_HESAB.hes, HEAD_LST.ANBARF, HEAD_LST_1"
        ".ANBAR HAVING (HEAD_LST.SADER = 0)"
    Caption ="فروش اشخاص"
...
End
CodeBehindForm
' See "R_FROOSH_DAYLY1_dakhel.cls"
